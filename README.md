...
- **관람인원별 가격 + 좌석 페어링**: 성인/청소년/경로/우대 인원수를 먼저 정하고, 좌석은 2명 단위로 실제 상영관처럼 (1,2)(3,4)(5,6)... 고정된 짝 단위로만 선택되도록(홀수면 마지막 1명은 자유 선택) 클라이언트에서 강제합니다. 예매 확정은 좌석 배열 + 인원수를 한 번에 서버로 보내 `prisma.$transaction`으로 묶어 처리하므로, 중간에 다른 사람이 좌석을 선점해도 이번 예매 전체가 롤백되어 일부 좌석만 예매되는 일이 없습니다. 좌석별 `category`/`price`는 이때 함께 저장됩니다.
...
- **좌석 페어링 규칙**: 2명 단위는 좌석이 (1,2)(3,4)(5,6)... 처럼 고정된 짝으로만 묶여, 2번 좌석을 클릭하면 항상 1번과 짝지어지고(3번과는 묶이지 않음) 3번을 클릭하면 항상 4번과 짝지어집니다(호버 시 짝 좌석까지 미리보기). 짝 좌석이 이미 예매돼 있으면 옆 좌석과 임의로 묶지 않고 안내 메시지를 보여줍니다. 홀수 인원의 마지막 1명은 좌석 1개만 자유롭게 선택
...
- **회차별 좌석 랜덤 시딩**: 시드 데이터에 회차마다 좌석의 약 30%를 미리 예약된 상태로 채워, 실제 서비스처럼 이미 예매된 좌석과 빈 좌석이 섞여 보이도록 함(전용 시드 계정이 소유). 좌석 페어링 규칙 테스트를 위해 `prisma/seed.ts`에서 현재는 주석 처리되어 있어, 재시딩하면 모든 회차가 빈 좌석으로 시작함

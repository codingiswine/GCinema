...
### 4. 테스트
```bash
npm test           # Jest + supertest, 실제 DB에 대해 핵심 플로우 검증 (39개)
...
```
prisma/
  schema.prisma          # User / Movie / Showtime / Booking / MovieLike / CheckoutAttempt 모델 (+ 결제수단·예매번호·취소 소프트삭제)
...
test/booking.test.ts         # 회원가입/로그인/예매/중복 검증 스모크 테스트 (39개)
...
- **테스트 범위도 최소화**: 전체 화면/라우트에 대한 테스트 대신, 실 서비스에서 버그가 나면 가장 치명적인 회원가입(아이디 형식, 이메일/휴대전화번호 중복 포함)/로그인/예매/중복예매/아이디 중복확인 로직에만 스모크 테스트(39개)를 두었습니다.
...
- **실제 영화관 같은 마이페이지 탭 구조**: 내가 본 영화 / 결제내역(예매내역·취소내역) / 나의 영화일기 / 1:1 문의 / 나의 정보관리 5개 탭으로 구성했습니다. 이 중 "내가 본 영화"(상영이 지난 활성 예매)와 "결제내역"(활성/취소 예매)은 이미 있는 `Booking` 데이터로 계산되는 실제 기능이고, 영화일기·1:1문의·정보관리는 과제 요구 기능과 무관한 새 영역이라 탭만 두고 "준비 중입니다" 안내만 보여줍니다(새 DB 테이블·CRUD 없이 화면 하나로 처리해 과제 범위를 넘지 않도록 함).

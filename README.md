...
### 4. 테스트
```bash
npm test           # Jest + supertest, 실제 DB에 대해 핵심 플로우 검증 (19개)
...
3. 영화 목록(포스터 카드) → 예매하기 (15/12세 영화는 연령등급 확인 팝업에서 확인 필요) → 포스터 클릭 시 상세 페이지에서도 확인 가능
4. 상영시간 선택 후 관람인원선택에서 성인/청소년/경로/우대 인원수 선택 (경로·우대는 안내 팝업 확인 필요)
...
src/
  ...
  views/
    ...
    partials/ageGateModal.ejs             # 연령등급(15/12세) 확인 팝업 (목록/상세/좌석선택 공용)
...
test/booking.test.ts         # 회원가입/로그인/예매/중복 검증 스모크 테스트 (19개)
...
- **연령등급 확인 팝업은 공용 partial로 분리**: 15/12세 관람가 영화는 예매 진입 지점(영화 목록 카드, 상세 페이지, 상영시간표 사이드바)마다 동일한 안내 팝업을 확인해야 다음 단계로 진행되는데, 이 로직을 `partials/ageGateModal.ejs` 하나로 묶어 재사용합니다. 좌석선택 화면의 최종 예매하기는 이미 진입 시점에 확인을 마쳤으므로 다시 띄우지 않습니다.
...
- **테스트 범위도 최소화**: 전체 화면/라우트에 대한 테스트 대신, 실 서비스에서 버그가 나면 가장 치명적인 회원가입(아이디 형식, 이메일/휴대전화번호 중복 포함)/로그인/예매/중복예매/아이디 중복확인 로직에만 스모크 테스트(19개)를 두었습니다.
...
- **상세 페이지 공유·좋아요**: 공유 버튼은 Web Share API(미지원 시 클립보드 복사)로 실제 공유 동작을 하고, 좋아요(하트)는 로그인 상태에서만 토글되며 비로그인 시 클릭하면 로그인 페이지로 이동
- **연령등급(15/12세) 확인 팝업**: 예매하기 버튼(목록 카드/상세 페이지/상영시간표 사이드바) 클릭 시 관람 등급 안내 팝업을 띄우고 확인해야 다음 단계로 진행 (좌석선택 최종 예매하기는 재확인 없이 바로 진행)
...
- **회차별 좌석 랜덤 시딩**: 시드 데이터에 회차마다 좌석의 약 30%를 미리 예약된 상태로 채워, 실제 서비스처럼 이미 예매된 좌석과 빈 좌석이 섞여 보이도록 함(전용 시드 계정이 소유)

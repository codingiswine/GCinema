...
- **박스오피스 랭킹 카드**: 영화 목록에 순위(1~4위), 포스터 위 순위 숫자와 함께 만족도·예매율·누적 관객수를 두 줄로 나눠 보여주고 카드마다 예매하기 버튼 배치
...
- **현재 위치를 알려주는 브레드크럼**: 헤더의 첫 항목은 영화 목록(`/movies`)으로 가는 홈 아이콘이고, 그 뒤로 단계에 따라 "→ 예매 → 좌석 선택"이 늘어나며 현재 화면의 위치를 표시
- **GCinema 브랜드 마크**: 레인보우 그라데이션(red-orange → green)을 적용한 로고 마크를 모든 화면 공통 헤더에 사용합니다. 영화 목록·상세의 만족도·예매율 줄에는 대신 좋아요 기능과 같은 하트 아이콘(빨간색)을 붙였습니다. 이 통계 줄은 "만족도·예매율"과 "누적 관객수"를 두 줄로 나눠 표시해서, 아이콘이 있어도 좁은 카드 폭에서 글자가 잘리지 않습니다.

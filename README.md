...
5. 좌석 그리드에서 선택한 인원 수만큼 좌석 클릭 → 2명 단위는 옆자리로 붙여서, 홀수 마지막 1명은 자유 선택 → 예매하기 (15/12세 영화는 여기서도 연령등급 확인 팝업 표시)
...
- **연령등급 확인 팝업은 공용 partial로 분리**: 15/12세 관람가 영화는 예매 진입 지점(영화 목록 카드, 상세 페이지, 상영시간표 사이드바, 좌석선택 최종 확정)마다 동일한 안내 팝업을 확인해야 다음 단계로 진행되는데, 이 로직을 `partials/ageGateModal.ejs` 하나로 묶어 4곳 모두 같은 마크업·문구를 재사용합니다.
...
- **연령등급(15/12세) 확인 팝업**: 예매하기 버튼(목록 카드/상세 페이지/상영시간표 사이드바/좌석선택 최종 확정) 클릭 시 관람 등급 안내 팝업을 띄우고 확인해야 다음 단계로 진행
